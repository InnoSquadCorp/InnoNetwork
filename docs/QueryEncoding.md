...
`EndpointBuilder` paths; a literal `?` or `#` is rejected with
`NetworkError.configuration(reason: .invalidRequest(...))`.
...
`EndpointBuilder.query(_:)` for builder-style endpoints. Raw spaces and non-ASCII

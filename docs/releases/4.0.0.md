...
- **`NetworkError` adds `.transportSuspended` and
  ...
  `en` and `ko`.
- **Resume compatibility guard.** The 4.0 implementation introduced a
  `StreamingResumeStrategy` marker around `StreamingResumePolicy`; the 5.0
  preview later removes that temporary public abstraction because no executor
  entry point accepted an alternate conformer.

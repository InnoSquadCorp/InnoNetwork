...
| Trigger | Status | Behavior in 5.x |
| --- | --- | --- |
...
| `Location` / `Content-Location` candidate URI invalidation | ❌ Not consumed | RFC 9111 marks these invalidations as MAY; 5.x limits the implementation to the mandatory target URI rule. |
...
- `307 Temporary Redirect` is not stored even though it is technically
  ...
  silently change observed redirect behaviour, so 5.x refuses
  the cache write.

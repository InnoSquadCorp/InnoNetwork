...
- **Background Download Info.plist.** 백그라운드 세션은 `UIBackgroundModes` 에 `fetch` 를(필요시
  `processing` 도) 선언해야 합니다.
...
- **실패 페이로드 캡처.** `NetworkError.objectMapping(payloadSnippet:)` 는 기본적으로 `nil` 이며,
  `NetworkConfiguration.captureFailurePayload = true` 로 명시적으로 활성화한 경우에만 채워집니다.
  PII 가 크래시 로그/분석에 남지 않도록 릴리즈 빌드에서는 비활성화 상태를 유지하세요.
...
- **로그아웃 시 cancel-all.** 로그아웃 / 계정 전환 / 백그라운딩 시 `DefaultNetworkClient.cancelAll()`
  을 호출합니다. 스트리밍 요청(SSE/NDJSON) 은 부모 task 가 cancel 될 때만 종료됩니다.

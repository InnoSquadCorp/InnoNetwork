...
InnoNetwork 는 Apple 플랫폼을 위한 타입 안전한 Swift 네트워킹 패키지입니다. 네 개의 공개 product
로 구성되어 있습니다.
...
`4.0.0` 은 최신 공개 릴리즈입니다. 일반적인 SwiftPM 사용에서는 `from: "4.0.0"` 을
사용하고, 아직 배포되지 않은 변경을 검증할 때만 이 저장소의 특정 리비전을 핀하세요.
...
- **인증 갱신.** `401` 이후 refresh + replay는 `ResponseInterceptor`가 아니라
  `RefreshTokenPolicy`로 설정합니다. 동시 refresh는 하나로 합쳐지고, 원 요청 replay는 최대 1회만
  수행됩니다.

...
InnoNetwork is responsible for **transport and session lifecycle**, not the application's
business workflow.

- Keep request/response APIs declarative and transport-focused.
- Model lifecycle-heavy modules such as download and websocket with explicit states.
- Let `InnoFlow` orchestrate domain and business phases above networking.
- Let `InnoRouter` own navigation replay or deep-link continuation decisions.

This avoids pushing one generic state-machine abstraction across the entire framework family.
...
Download/WebSocket은 상태머신 성격이 강한 영역입니다. 이 모듈들은 enum 상태와 전이 불변식을
명시하지만, 일반 `APIDefinition` 요청 모델 전체를 범용 오토마타 프레임워크로 일반화하지는
않습니다.

You can inspect lifecycle invariants directly in code with `nextStates`, `isTerminal`, and
`canTransition(to:)` on `DownloadState` and `WebSocketState`.

## Requirements

- Swift 6.2+
- iOS 26.0+ / macOS 14.0+ / tvOS 26.0+ / watchOS 26.0+ / visionOS 26.0+

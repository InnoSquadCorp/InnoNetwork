...
in-memory request bodies so application teams can validate one signing path
without pulling AWS-specific policy into the core `InnoNetwork` product.
...
The signer supports normal header-based SigV4 for requests whose body is
already available as `Data`. Streaming SigV4 payload signing, presigned URLs,
credential provider chains, automatic rotation, and service-specific AWS SDK

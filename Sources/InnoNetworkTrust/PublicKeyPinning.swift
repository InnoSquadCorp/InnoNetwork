...
/// ``InnoNetworkTrust`` and wrap the policy with
...
///     pinsByHost: ["api.example.com": ["sha256/AAAA..."]]
/// )
/// let evaluator = PublicKeyPinningEvaluator(policy: policy)
/// let trustPolicy = TrustPolicy.custom(evaluator)
/// ```
...
public struct PublicKeyPinningEvaluator: TrustEvaluating {
    ...
    public init(policy: PublicKeyPinningPolicy) {
        self.policy = policy
    }

    public func evaluate(challenge: URLAuthenticationChallenge) -> TrustChallengeOutcome {
    ...
    /// Encodes the SubjectPublicKeyInfo (SPKI) DER for the supplied public key.
    ///
    /// Exposed at `package` scope so the package's own test target can
    /// validate the encoding for known vectors. Returning `nil` indicates
    /// the algorithm is unsupported and the caller should fall back to
    /// hashing the raw public-key bytes.
    package static func spkiData(
        publicKeyData: Data,
        keyType: String,
        keySizeInBits: Int
    ) -> Data? {
